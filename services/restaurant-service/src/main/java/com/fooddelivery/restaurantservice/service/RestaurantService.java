...
public class RestaurantService {
  ...
  public RestaurantResponse create(CreateRestaurantRequest req) {
    var restaurant =
        new Restaurant(UUID.randomUUID().toString(), req.name(), req.avgPrepTimeMinutes());
  ...
  /**
   * Called by OrderEventConsumer when an order-created event arrives. Looks up the restaurant's
   * avgPrepTimeMinutes and publishes order-accepted. If the restaurant is closed or not found, the
   * order is silently dropped (a real implementation would publish order-rejected — added in a
   * later phase).
   */
  @Transactional(readOnly = true)
